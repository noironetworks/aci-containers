...
make -C . all-static all-static-race
...
docker images
docker build -t ${QUAY_REGISTRY}/aci-containers-controller:${IMAGE_TAG} --build-arg binarypath=dist-static-race --file=docker/travis/Dockerfile-controller .
docker images
docker build -t ${QUAY_REGISTRY}/aci-containers-host:${IMAGE_TAG} ---build-arg binarypath=dist-static-race --file=docker/travis/Dockerfile-host .
docker images
docker build -t ${QUAY_REGISTRY}/aci-containers-operator:${IMAGE_TAG} ---build-arg binarypath=dist-static-race --file=docker/travis/Dockerfile-operator .
docker images
