...
func loginWithTLS(apicClient *http.Client, apicHosts []string, certFile, keyFile string) (string, int, error) {
...
func apicLogin(apicClient *http.Client, apicHosts []string, apicUser, apicPassword, certFile, keyFile string, aciConfig *acictrlr.ControllerConfig) (string, int, error) {

	if certFile != "" && keyFile != "" {
		return loginWithTLS(apicClient, apicHosts, certFile, keyFile)
	}
