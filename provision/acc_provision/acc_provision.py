    ...
    lower_in = lambda y: (
        lambda x: (
            (True if str(x).lower() in y
                                  ...
                                  "Expected one of: {%s}" %
                                  (x, ','.join(y)))))))
    isname = lambda x: (1 < len(x[0]) < x[1]) and \
        x[0][0].isalpha() and x[0].replace('_', '').isalnum() \
        if x[0] else Raise(Exception("Missing option"))
    get = lambda t: functools.reduce(lambda x, y: x and x.get(y), t, config)

    checks = {
        # ACI config
        "aci_config/system_id": (get(("aci_config", "system_id")), required),
        "aci_config/system_id/type": (((get(("aci_config", "system_id"))), 32), isname),
