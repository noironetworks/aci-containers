...
type AciContainersOperatorLister interface {
	...
	List(selector labels.Selector) (ret []*v1.AciContainersOperator, err error)
...
// List lists all AciContainersOperators in the indexer.
func (s *aciContainersOperatorLister) List(selector labels.Selector) (ret []*v1.AciContainersOperator, err error) {
	err = cache.ListAll(s.indexer, selector, func(m interface{}) {
		ret = append(ret, m.(*v1.AciContainersOperator))
	})
	return ret, err
}
...
type AciContainersOperatorNamespaceLister interface {
	...
	// Objects returned here must be treated as read-only.
	Get(name string) (*v1.AciContainersOperator, error)
...
// List lists all AciContainersOperators in the indexer for a given namespace.
func (s aciContainersOperatorNamespaceLister) List(selector labels.Selector) (ret []*v1.AciContainersOperator, err error) {
...
// Get retrieves the AciContainersOperator from the indexer for a given namespace and name.
func (s aciContainersOperatorNamespaceLister) Get(name string) (*v1.AciContainersOperator, error) {
	obj, exists, err := s.indexer.GetByKey(s.namespace + "/" + name)
	if err != nil {
		return nil, err
	}
	if !exists {
		return nil, errors.NewNotFound(v1.Resource("acicontainersoperator"), name)
	}
	return obj.(*v1.AciContainersOperator), nil
}
