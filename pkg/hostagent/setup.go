...
func (agent *HostAgent) configureContainerIfaces(metadata *md.ContainerMetadata) (*cnicur.Result, error) {
	...
	//	var deviceId string
	//	agent.log.Debug(metadata.Id.Pod + "_" + metadata.Id.Namespace)
	////	if agent.deviceIdMap[metadata.Id.Pod+"_"+metadata.Id.Namespace] != nil {
	//		deviceIdList = agent.deviceIdMap[metadata.Id.Pod+metadata.Id.Namespace]
	//		agent.log.Debug(deviceIdList)
	//		if len(deviceIdList) == 1 {
	//			agent.log.Debug(deviceId)
	//			deviceId = strings.Join(deviceIdList, " ")
	//		}
	//	}

	deviceId, err := agent.getNetAttachment(metadata.Id.Pod, metadata.Id.Namespace)
	if err != nil {
		agent.log.Debug("VF allocation failed")
	}
			...
			if ip.Address.IP != nil && ip.Address.IP.To4() != nil {
				if deviceId != "" {
					...
					iface.HostVethName, iface.Mac, iface.VfNetDevice, err =
						runSetupVf(iface.Sandbox, iface.Name, mtu, ip.Address.IP, deviceId)
...
func (agent *HostAgent) unconfigureContainerIfaces(id *md.ContainerId) error {
	...
	deviceId, err := agent.getNetAttachment(metadata.Id.Pod, metadata.Id.Namespace)
        if err != nil {
                agent.log.Debug("VF deallocation failed")
        }

	for _, iface := range metadata.Ifaces {
		if deviceId != "" {
			logger.Debug("renaming and moving the VF back to host's namespace")
			err = runClearVf(iface.Sandbox, iface.Name, deviceId, iface.VfNetDevice)
			if err != nil {
