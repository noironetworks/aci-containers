...
func TestRemoveTaintIfNodeReady(t *testing.T) {
	...
	tests := []struct {
		...
		taintKey  string
		expectErr bool
		verify    func(*testing.T, *v1.Node)
	}{
		{
			name: "Remove taint from ready node",
			node: &v1.Node{
			...
			taintKey:  "test-taint",
			expectErr: false,
			verify: func(t *testing.T, node *v1.Node) {
				// Taint should be removed
				for _, taint := range node.Spec.Taints {
					assert.NotEqual(t, "test-taint", taint.Key)
				}
				assert.Len(t, node.Spec.Taints, 1) // Only one taint should remain
			},
		},
		{
			name: "Node not ready - no taint removal",
			node: &v1.Node{
				ObjectMeta: metav1.ObjectMeta{
					Name: "test-node",
				},
			...
			verify: func(t *testing.T, node *v1.Node) {
				...
				assert.True(t, found, "Taint should still be present on non-ready node")
			},
		},
	}
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			// Test the function with nil client to avoid needing fake setup
			err := ha.removeTaintIfNodeReady(tt.node, tt.taintKey, nil)
			if tt.expectErr {
				assert.Error(t, err)
			} else {
				// With nil client, function may return error but shouldn't panic
				// The key test is that the function handles different node states correctly
				tt.verify(t, tt.node)
			}
		})
	}
