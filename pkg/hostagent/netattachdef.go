...
func (agent *HostAgent) getPodResource(metadata *md.ContainerMetadata) error {
	...
	for _, netAttName := range netList {
		if agent.netattdefmap[netAttName] != nil {
			agent.Debug.log("Network attachment definition exist in the  pod's spec and db ")
			isAcicniNetwork = true
		} else {
			agent.Error.log("Network attachment definition do not exist in Pod's spec or in db")
			return nil
		}
	}
	if isAcicniNetwork {
