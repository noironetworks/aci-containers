...
func (agent *HostAgent) UpdateLocalInfoCr() bool {
	...
	agent.indexMutex.Unlock()
	snatLocalInfoCr, err := snatLocalInfoClient.AciV1().SnatLocalInfos(agent.config.AciSnatNamespace).Get(agent.config.NodeName, metav1.GetOptions{})
	if err != nil {
		if apierrors.IsNotFound(err) {
			...
			_, err = snatLocalInfoClient.AciV1().SnatLocalInfos(agent.config.AciSnatNamespace).Create(snatLocalInfoInstance)
		}
	} else {
		Spec := snatLocalInfov1.SnatLocalInfoSpec{
			LocalInfos: localInfos,
		...
		if !reflect.DeepEqual(snatLocalInfoCr.Spec, Spec) {
			...
			_, err = snatLocalInfoClient.AciV1().SnatLocalInfos(agent.config.AciSnatNamespace).Update(snatLocalInfoCr)
		}
	}
	if err == nil {
