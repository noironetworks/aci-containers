...
type AccProvisionInputLister interface {
	...
	List(selector labels.Selector) (ret []*v1.AccProvisionInput, err error)
...
// List lists all AccProvisionInputs in the indexer.
func (s *accProvisionInputLister) List(selector labels.Selector) (ret []*v1.AccProvisionInput, err error) {
	err = cache.ListAll(s.indexer, selector, func(m interface{}) {
		ret = append(ret, m.(*v1.AccProvisionInput))
	})
	return ret, err
}
...
type AccProvisionInputNamespaceLister interface {
	...
	// Objects returned here must be treated as read-only.
	Get(name string) (*v1.AccProvisionInput, error)
...
// List lists all AccProvisionInputs in the indexer for a given namespace.
func (s accProvisionInputNamespaceLister) List(selector labels.Selector) (ret []*v1.AccProvisionInput, err error) {
...
// Get retrieves the AccProvisionInput from the indexer for a given namespace and name.
func (s accProvisionInputNamespaceLister) Get(name string) (*v1.AccProvisionInput, error) {
	obj, exists, err := s.indexer.GetByKey(s.namespace + "/" + name)
	if err != nil {
		return nil, err
	}
	if !exists {
		return nil, errors.NewNotFound(v1.Resource("accprovisioninput"), name)
	}
	return obj.(*v1.AccProvisionInput), nil
}
