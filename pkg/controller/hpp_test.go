...
// hppChanged/hppDeleted/hppRemoteIpChanged/hppRemoteIpDeleted only touch
// cont.remIpContQueue.
...
func TestHppChanged(t *testing.T) {
	...
	// hppChanged routes through queueRemoteIpConUpdateByKey, which adds
	// to remIpContQueue -- same destination queue as hppRemoteIpChanged,
	// just via an indirect helper rather than a direct queue.Add call.
	item := getQueueItem(t, cont.remIpContQueue)
	assert.Equal(t, "test-hpp-1", item)
	assert.Equal(t, 0, cont.hppQueue.Len())
}
...
func TestHppRemoteIpChanged(t *testing.T) {
	...
	// hppRemoteIpChanged adds directly to remIpContQueue -- same
	// destination as hppChanged (via queueRemoteIpConUpdateByKey), so
	// the two code paths are NOT asymmetric in effect, only in how they
...
// AddEventHandler wiring, ending up on remIpContQueue.
