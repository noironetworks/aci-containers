...
const (
	...
	nadNameAnnotationKey       = "nad"
	namespaceNameAnnotationKey = "namespace"
)
...
func aaepMonitorInit(cont *AciController, stopCh <-chan struct{}) {
	cont.log.Debugf("Initializing aaepMonitorClient")
...
func (cont *AciController) aaepMonitorConfAdded(obj interface{}) {
	...
	cont.log.Infof("AaepMonitorConfig added: %s", aaepMonitorConfig.Name)
...
func (cont *AciController) aaepMonitorConfUpdate(oldobj interface{}, newobj interface{}) {
	...
	cont.log.Infof("AaepMonitorConfig updated: %s", newAaepMonitorConfig.Name)
...
func (cont *AciController) aaepMonitorConfDelete(obj interface{}) {
	...
	aaepMonitorConfig, ok := obj.(*amv1.AaepMonitor)
	cont.log.Infof("AaepMonitorConfig deleted: %s", aaepMonitorConfig.Name)
...
func (cont *AciController) handleAaepMonitorConfigurationUpdate(obj interface{}) bool {
	cont.log.Infof("[handleAaepMonitorConfigurationUpdate] Handling AAEP monitoring configuration creation/updation: %v", cont.sharedAaepMonitor)
	aaepMonitorConfig, ok := obj.(*amv1.AaepMonitor)
	if !ok {
	...
	cont.log.Infof("Exisintg map ", cont.sharedAaepMonitor)
	cont.log.Infof("Added AAEPs: %v and Removed AAEPs: %v", addedAaeps, removedAaeps)
	for _, aaepName := range addedAaeps {
		if !cont.reconcileNadData(aaepName) {
			cont.indexMutex.Lock()
			cont.sharedAaepMonitor[aaepName] = []*AaepMonitoringData{}
			cont.indexMutex.Unlock()
			cont.log.Infof("sharedAaepMonitor if reconcile fails: %v", cont.sharedAaepMonitor)
			continue
		}
		cont.log.Infof("Map after reconcile: %v %s", cont.sharedAaepMonitor, aaepName)
		aaepEpgDataList := cont.sharedAaepMonitor[aaepName]
		for _, aaepEpgData := range aaepEpgDataList {
			cont.createNetworkAttachmentDefinition(aaepEpgData)
		}
	}
	...
	for _, aaepName := range removedAaeps {
		...
		for _, aaepEpgData := range aaepEpgDataList {
			cont.deleteNetworkAttachmentDefinition(aaepEpgData)
		}
	}
...
func (cont *AciController) handleAaepMonitorConfigurationDelete(key string) bool {
	cont.log.Infof("Handling AAEP monitoring configuration deletion")
	...
	for aaepName, aaepMonitorDataList := range cont.sharedAaepMonitor {
		...
		for _, aaepMonitorData := range aaepMonitorDataList {
			cont.deleteNetworkAttachmentDefinition(aaepMonitorData)
		}
	}
...
func (cont *AciController) handleAaepEpgAttach(infraRsObj apicapi.ApicObject) {
	cont.log.Debugf("[AAEP-HANDLER] Monitoring Data before attach =", cont.sharedAaepMonitor)
	infraRsObjDn := infraRsObj.GetDn()
	// Check if DN contains any of the AAEPs from the list
	aaepName := cont.matchesAEPFilter(infraRsObjDn)
	if aaepName == "" {
		cont.log.Debugf("[AAEP-HANDLER] Unable to find AAEP from %s in monitoring list", infraRsObjDn)
		return
	}
	cont.log.Debugf("[AAEP-HANDLER] EPG attached to AAEP in list: AAEP=", aaepName, " DN=", infraRsObjDn)
	...
	cont.log.Debugf("[AAEP-HANDLER] EPG DN=", epgDn)
	...
	aaepMonitorData := cont.collectNadData(aaepName, aaepEpgData)
	if aaepMonitorData == nil {
		return
	}

	oldAaepMonitorData, dataIndex := cont.getAaepEpgAttachData(aaepName, epgDn)
	cont.syncNADsWithAciState(aaepName, dataIndex, oldAaepMonitorData, aaepMonitorData)

	cont.log.Debugf("[AAEP-HANDLER] Monitoring Data After attach =", cont.sharedAaepMonitor)
}

func (cont *AciController) handleAaepEpgDetach(infraRsObjDn string) {
	cont.log.Debugf("[AAEPDETACH-HANDLER] EPG State 2: ", cont.sharedAaepMonitor)
	cont.log.Debugf("[AAEPDETACH-HANDLER] EPG detached to AAEP: DN=", infraRsObjDn)
	...
	aaepMonitorData, dataIndex := cont.getAaepEpgAttachData(aaepName, epgDn)
	...
	cont.indexMutex.Lock()
	cont.log.Infof("NS name %s and NAD name: %s", aaepMonitorData.namespaceName, aaepMonitorData.nadName)
	cont.deleteNetworkAttachmentDefinition(aaepMonitorData)
	...
	cont.log.Debugf("[AAEPDETACH-HANDLER] EPG State 222: ", cont.sharedAaepMonitor)
}

func (cont *AciController) handleAnnotationAdded(obj apicapi.ApicObject) bool {
	cont.log.Debugf("[handleAnnotationAdded] Monitoring Data Before handleAnnotationAdded =", cont.sharedAaepMonitor)
	...
	cont.log.Infof("[handleAnnotationAdded] Inside handleAnnotationAdded: EPG %s, AAEP %s, Annotation: %s", epgDn, aaepName, annotationDn)
	if aaepMonitorData == nil || !cont.namespaceChecks(aaepMonitorData.namespaceName, epgDn) {
		cont.log.Debugf("[handleAnnotationAdded] Insufficient data for NAD creation: Either namespace not exist or monitoring data not found")
		return true
	}

	cont.log.Infof("[handleAnnotationAdded] NAD name: %s", aaepMonitorData.nadName)
	oldAaepMonitorData, dataIndex := cont.getAaepEpgAttachData(aaepName, epgDn)

	cont.log.Infof("[handleAnnotationAdded] oldAaepMonitorData : %v and Index: %d", oldAaepMonitorData, dataIndex)
	cont.syncNADsWithAciState(aaepName, dataIndex, oldAaepMonitorData, aaepMonitorData)

	cont.log.Debugf("[handleAnnotationAdded] Monitoring Data After handleAnnotationAdded %v", cont.sharedAaepMonitor)
	return true
}

func (cont *AciController) handleAnnotationDeleted(annotationDn string) {
	cont.log.Debugf("[AAEP-HANDLER] Monitoring Data Before handleAnnotationDeleted = %v", cont.sharedAaepMonitor)
	epgDn := annotationDn[:strings.Index(annotationDn, "/annotationKey-")]
	...
	cont.log.Infof("oldAaepMonitorData : %v and Index: %d", oldAaepMonitorData, dataIndex)
	...
	if aaepMonitorData == nil {
		cont.indexMutex.Lock()
		cont.deleteNetworkAttachmentDefinition(oldAaepMonitorData)
	...
	cont.log.Debugf("[AAEP-HANDLER] Monitoring Data After handleAnnotationAdded = %v", cont.sharedAaepMonitor)
}

func (cont *AciController) collectNadData(aaepName string, aaepEpgData *AaepEpgAttachData) *AaepMonitoringData {
	...
	cont.log.Infof("NS name %s and NAD name: %s", namespaceName, nadName)
	if !cont.namespaceChecks(namespaceName, epgDn) {
		return nil
	}

	if nadName == "" {
		nadName = cont.generateDefaultNadName(epgDn)
		cont.log.Debugf("Proceeding with default NAD name %s. NAD name not provided in EPG %s annotations", nadName, epgDn)
	}
...
func (cont *AciController) findAaepEpgAttachDataInCache(epgDn string) (*AaepMonitoringData, int) {
	for aaepName, _ := range cont.sharedAaepMonitor {
		aaepEpgData, dataIndex := cont.getAaepEpgAttachData(aaepName, epgDn)
		if aaepEpgData != nil {
...
func (cont *AciController) getAaepEpgAttachData(aaepName string, epgDn string) (*AaepMonitoringData, int) {
	...
	for dataIndex, aaepEpgData := range aaepEpgDataList {
		if aaepEpgData.aaepEpgData.epgDn == epgDn {
			cont.log.Infof("[getAaepEpgAttachData] Found Attach data: %v EPG : %s AAEP: %s", aaepEpgData, epgDn, aaepName)
			return aaepEpgData, dataIndex
		}
	}
	cont.log.Infof("[getAaepEpgAttachData] Not found attach data EPG : %s AAEP: %s", epgDn, aaepName)
	return nil, -1
}
...
func (cont *AciController) getAaepMonitoringDataForEpg(epgDn string) (string, *AaepMonitoringData) {
	...
	var aaepName string
	cont.log.Infof("[getAaepMonitoringDataForEpg] before get %v", cont.sharedAaepMonitor)
	// for aaepName = range cont.sharedAaepMonitor {
	// 	aaepEpgDataList := cont.getAaepEpgAttObjDetails(aaepName)

	// 	if aaepEpgDataList == nil {
	// 		cont.log.Infof("DATA LIST NIL")
	// 		continue
	// 	}

	// 	for _, aaepEpgData := range aaepEpgDataList {
	// 		if aaepEpgData.epgDn == epgDn {
	// 			aaepMonitorData = cont.collectNadData(aaepName, &aaepEpgData)
	// 			cont.log.Infof("[getAaepMonitoringDataForEpg] found monitoring data %v current map %v AAEP %s EPG %s", aaepMonitorData, cont.sharedAaepMonitor, aaepName, epgDn)
	...
	cont.log.Infof("[getAaepMonitoringDataForEpg] not found monitoring data current map %v AAEP %s EPG %s", cont.sharedAaepMonitor, aaepName, epgDn)
	return "", nil
}
...
func (cont *AciController) syncNADsWithAciState(aaepName string, dataIndex int, oldAaepMonitorData, aaepMonitorData *AaepMonitoringData) {
	if oldAaepMonitorData == nil {
		cont.log.Debugf("[syncNADsWithAciState] OLD MONT DATA: %v", oldAaepMonitorData)
		cont.createNetworkAttachmentDefinition(aaepMonitorData)
		cont.log.Infof("[syncNADsWithAciState] After createNetworkAttachmentDefinition %v", aaepMonitorData)
		cont.sharedAaepMonitor[aaepName] = append(cont.sharedAaepMonitor[aaepName], aaepMonitorData)
	} else {
		if oldAaepMonitorData.namespaceName != aaepMonitorData.namespaceName || oldAaepMonitorData.nadName != aaepMonitorData.nadName || oldAaepMonitorData.aaepEpgData.encapVlan != aaepMonitorData.aaepEpgData.encapVlan {
			cont.log.Debugf("[syncNADsWithAciState] Change detected for EPG ",
				" oldNamespace=", oldAaepMonitorData.namespaceName, " oldNad=", oldAaepMonitorData.nadName, " oldVlan=", oldAaepMonitorData.aaepEpgData.encapVlan, " newNamespace=", aaepMonitorData.namespaceName, " newNad=", aaepMonitorData.nadName, " newVlan=", aaepMonitorData.aaepEpgData.encapVlan)
	...
	cont.log.Infof("[syncNADsWithAciState] Completed %v", cont.sharedAaepMonitor)
}

func (cont *AciController) addDeferredNADs(namespaceName string) {
	cont.log.Infof("Inside processDefferedNADs map: %v NS: %s", cont.sharedAaepMonitor, namespaceName)
	cont.indexMutex.Lock()
	defer cont.indexMutex.Unlock()
	for aaepName, _ := range cont.sharedAaepMonitor {
		aaepEpgDataList := cont.getAaepEpgAttObjDetails(aaepName)
		...
		for _, aaepEpgData := range aaepEpgDataList {
			aaepMonitoringData := cont.collectNadData(aaepName, &aaepEpgData)
			if aaepMonitoringData == nil || aaepMonitoringData.namespaceName != namespaceName {
				continue
			}

			cont.createNetworkAttachmentDefinition(aaepMonitoringData)
...
func (cont *AciController) getAaepEpgAttObjDetails(aaepName string) []AaepEpgAttachData {
	cont.log.Infof("Inside aaepEpgAttchDetails")
	...
	if len(resp.Imdata) == 0 {
		cont.log.Debugf("Skipping NAD creation: Can't find EPGs attached with AAEP ", aaepName)
		return nil
	}
	...
	for _, respImdata := range resp.Imdata {
		...
		cont.log.Infof("aaepEpgAttchDetails aaepEpgAttachObj: ", aaepEpgAttachObj)
	...
	cont.log.Infof("getAaepEpgAttObjDetails MAP: ", aaepEpgAttchDetails)
...
func (cont *AciController) reconcileNadData(aaepName string) bool {
	aaepEpgDataList := cont.getAaepEpgAttObjDetails(aaepName)
	...
	for _, aaepEpgData := range aaepEpgDataList {
		...
		aaepMonitoringData := cont.collectNadData(aaepName, &aaepEpgData)
		cont.log.Infof("Monitoring data after collectNadData: %v", aaepMonitoringData)
...
func (cont *AciController) createNetworkAttachmentDefinition(nadData *AaepMonitoringData) {
	bridge := cont.config.BridgeName // Linux bridge name
	if bridge == "" {
			...
			if json.Unmarshal([]byte(existingConfig), &existingCNVConfig) == nil {
				if existingVLAN, ok := existingCNVConfig["vlan"].(float64); ok {
					if int(existingVLAN) == vlanID {
						...
						cont.log.Infof("NetworkAttachmentDefinition %s already exists with correct VLAN %d in namespace %s", nadName, vlanID, namespaceName)
						return
					}
				} else if vlanID == 0 {
					// Both existing and new have no VLAN, no update needed
					cont.log.Infof("NetworkAttachmentDefinition %s already exists with no VLAN in namespace %s", nadName, namespaceName)
					return
				}
			}
		}

		// VLAN changed or couldn't parse existing config, update NAD
		cont.log.Infof("Updating NetworkAttachmentDefinition %s in namespace %s (VLAN: %d)", nadName, namespaceName, vlanID)
	}

	cnvBridgeConfig := map[string]interface{}{
		"cniVersion":                "0.3.1",
		"name":                      nadName,
		"type":                      "bridge",
	...
	if err != nil {
		cont.log.Errorf("failed to marshal CNV bridge config: ", err)
		return
	}
	...
	if nadExists {
		...
		if err != nil {
			cont.log.Errorf("failed to update NetworkAttachmentDefinition: ", err)
			return
		}
		cont.log.Infof("Updated NetworkAttachmentDefinition: %s in namespace %s (VLAN: %d)", nadName, namespaceName, vlanID)
	} else {
		_, err = nadClient.K8sCniCncfIoV1().NetworkAttachmentDefinitions(namespaceName).Create(context.TODO(), nad, metav1.CreateOptions{})
		if err != nil {
			cont.log.Errorf("failed to create NetworkAttachmentDefinition: ", err)
			return
		}
		cont.log.Infof("Created NetworkAttachmentDefinition: %s in namespace %s (VLAN: %d)", nadName, namespaceName, vlanID)
	}

	return
}

func (cont *AciController) deleteNetworkAttachmentDefinition(nadData *AaepMonitoringData) {
	namespaceName := nadData.namespaceName
	...
	if nadExists {
		...
		if cont.isNADinUse(namespaceName, nadName) {
			...
			if err != nil {
				cont.log.Errorf("Failed to add out-of-sync annotation to the NAD: ", err)
				return
			}
			cont.log.Infof("Added annotation out-of-sync for NAD : %s in namespace %s", nadName, namespaceName)
			return
		}
		...
		if err != nil {
			cont.log.Errorf("Failed to remove VMM lite annotation from NetworkAttachmentDefinition: ", err)
			return
		}
		cont.log.Infof("Removed VMM lite annotation from NetworkAttachmentDefinition: %s in namespace %s", nadName, namespaceName)
...
func (cont *AciController) getAaepDiff(crAaeps []string) (addedAaeps, removedAaeps []string) {
	...
	for cachedAaep, _ := range cont.sharedAaepMonitor {
...
func (cont *AciController) getEncapFromAaepEpgAttachObj(aaepName, epgDn string) string {
	...
	for _, obj := range resp.Imdata {
		...
		if !ok {
			cont.log.Errorf("getEncapFromAaepEpgAttachObj: infraRsFuncToEpg Object not found in response")
			break
		}
		if val, ok := lresp.Attributes["encap"]; ok {
			...
			cont.log.Errorf("getEncapFromAaepEpgAttachObj: encap missing for infraRsFuncToEpg %s: %v", uri, err)
			break
		}
	}
...
func (cont *AciController) isVmmLiteNAD(nadDetails *nadapi.NetworkAttachmentDefinition) bool {
	cont.log.Infof("NAD details: %v", nadDetails)
	if nadDetails.ObjectMeta.Annotations["managed-by"] == "cisco-network-operator" {
		return true
	}
	return false
}
