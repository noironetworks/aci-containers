...
/*
func (cont *AciController) getPCName(nodeName string, mbrIf string) (string, bool) {
    cont.indexMutex.Lock()
    for idx := range cont.AggIfCache[nodeName] {
...
func (cont *AciController) updateNodeFabNetAttObj(nodeFabNetAtt *fabattv1.NodeFabricNetworkAttachment) apicapi.ApicSlice {
	...
	epg := apicapi.NewFvAEPg(cont.config.AciPolicyTenant, cont.config.AciPolicyTenant, addNet.NetworkName)
...
func (cont *AciController) nodeFabNetAttDeleted(obj interface{}) bool {
	...
	nodeCache, ok := addNet.NodeCache[nodeFabNetAtt.Spec.NodeName]
	if !ok {
		return true
	}
	delete(addNet.NodeCache, nodeFabNetAtt.Spec.NodeName)
	...
	for iface := range nodeCache.Spec.AciTopology {
		if _, ok := addNet.FabricLink[nodeFabNetAtt.Spec.NodeName]; !ok {
			break
		}
		localIfaceMap := addNet.FabricLink[nodeFabNetAtt.Spec.NodeName]
		delete(localIfaceMap, iface)
		addNet.FabricLink[nodeFabNetAtt.Spec.NodeName] = localIfaceMap
	}
	bd := apicapi.NewFvBD(cont.config.AciPolicyTenant, addNet.NetworkName)
	bd.SetAttr("arpFlood", "yes")
	bd.SetAttr("ipLearning", "no")
