...
func TestInitStaticNetPolObjs(t *testing.T) {
	initCont := func() *testAciController {
		cont := testController()
		cont.config.EnableLocalNetpol = true
		cont.config.AciPolicyTenant = "test-tenant"
		cont.config.NodeServiceIpPool = []ipam.IpRange{
			{Start: net.ParseIP("10.1.1.2"), End: net.ParseIP("10.1.1.3")},
		}
	...
	cont.config.EnableLocalNetpol = false
...
func TestIngressEgressStr(t *testing.T) {
	prot := v1.Protocol("tcp")
	ports := []v1net.NetworkPolicyPort{
		{
			Protocol: &prot,
			Port: &intstr.IntOrString{
	...
	peers := []v1net.NetworkPolicyPeer{
		{
			IPBlock: &v1net.IPBlock{
				...
				Except: []string{"192.168.10.0", "192.168.10.255"},
			},
		},
	}
	...
	expected := "[192.168.10.0/24[except-192.168.10.0-192.168.10.255]]:[tcp:8080]"
