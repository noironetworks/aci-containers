...
func (cont *AciController) qosPolicyDelete(qosobj interface{}) {
	...
	if err != nil {
                QosPolicyLogger(cont.log, qr).
                        Error("Could not create policing policy key: ", err)
                return
        }
...
func (cont *AciController) handleQosPolUpdate(qp *qospolicy.QosPolicy) bool {
	...
	DppPolEgress:=
        apicapi.NewQosDppPol(cont.config.AciPolicyTenant, "egress")
	qrDn := qr.GetDn()
	...
	cont.apicConn.WriteApicObjects(labelKey, apicapi.ApicSlice{qr, DppPolIngress, DppPolEgress})
